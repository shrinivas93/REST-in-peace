 ...
 *
 * <p>
 * Keyed by {@link #key(HTTPMethod, String)} - {@code "<HTTP method> <exact
 * request URL, query string included>"} (e.g.
 * {@code "GET https://api.example.com/items?page=2"}) - a single slot per
 * key, the newest variant replacing the previous one, rather than storing
 * every {@code Vary}-distinguished variant at once. A response naming a
...
public interface Cache {
	 ...
	 *
	 * <p>
	 * {@code url} must be the <em>exact</em> absolute URL as sent on the
	 * wire, query string included in the same order/encoding RIP itself
	 * would produce (e.g. from {@code @QueryParam}/{@code @QueryMap} applied
	 * in method-declaration order) - a mismatched query string computes a
	 * different key and silently won't evict anything. For a method with no
	 * query parameters at all, this is simply the interface method's
	 * resolved URL.
	 *
	 ...
	 * @param url         the exact absolute request URL, query string included
